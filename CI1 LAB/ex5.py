...
def create_world():
    n_input = input("Enter number of rooms (>=4): ")
    try:
        n = int(n_input)
    except ValueError:
        n = 4
...
def display_world(world, n, agent_pos, agent_dir):
    ...
    for i in range(1, n+1):
        for j in range(1, n+1):
            ...
            # Show Agent icon if current cell matches agent position
            if (i, j) == agent_pos:
                print(f"A{icons[agent_dir]}", end="\t")
            elif cell["wampus"]:
                print("W", end="\t")
            elif cell["pit"]:
                print("P", end="\t")
...
def play_game():
    ...
    while not game_over:
        # Display the map with the agent 'A'
        display_world(world, n, position, direction)
